#!/usr/bin/env python3
"""
V4.0 Training Script: Dual-Stream Architecture
ResNet1D (Classification) + UNet (Segmentation)
"""
...
def train(args):
    ...
    # 2. Model: Dual-Stream
    print("Creating Dual-Stream Model (v4.0)...")
    # Branch A: ResNet1D (Classification)
    # Branch B: UNet-SE (Segmentation)
    model = create_model('dual', input_length=8000, 
                        n_classes_seg=5, n_classes_clf=5, 
                        in_channels=34, attention=True).to(device)
    
    # 3. Optimization
    # We train both branches jointly
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, verbose=True)
    
    # Losses
    criterion_clf = nn.CrossEntropyLoss()
    # Weighted CE for masks (Artifacts are rare)
    ...
    print("Starting Training (Dual-Stream)...")
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Acc/val_seg', val_acc_seg, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_dual_v4')
    parser.add_argument('--log_dir', type=str, default='runs_dual_v4')
